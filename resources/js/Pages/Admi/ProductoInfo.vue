...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <h2 class="font-semibold text-xl text-black leading-tight">Información del producto</h2>
            ...
            <table>
                <tbody>
                    <tr>
                        ...
                        <td>{{ producto.nombre_producto ?? null }}</td>
                    </tr>
                        ...
                        <td>
                            <span
                                class="bg-primary-100 text-primary-800 text-xs font-medium px-2 py-0.5 rounded dark:bg-primary-900 dark:text-primary-300">{{
                                    categoria.nombre_categoria ?? null }}</span>
                        </td>
                    </tr>
                    <tr class="info-container">
                    ...
                    <tr class="info-container">
                        <td class="label">Cantidad mínima de existencia</td>
        ...
        <cardLayout>
            ...
            <tablaCRUDLayout>
                <template #tituloTabla>
                    <tituloTabla>Proveedor</tituloTabla>
                    <tituloTabla>Cantidad</tituloTabla>
                    <tituloTabla>Precio</tituloTabla>
                    <tituloTabla>Fecha de entrada</tituloTabla>
                    <tituloTabla>Fecha de vencimiento</tituloTabla>
                </template>
                ...
                <template #contenidoTabla>
                    <tr v-for="(p, index) in proveedores " :key="index">
                        ...
                        <contenidoTabla>{{ p.pivot.cantidad ?? null }}</contenidoTabla>
                        <contenidoTabla>{{ p.pivot.precio ?? null }}</contenidoTabla>
                        <contenidoTabla>{{ p.pivot.fecha_entrada ?? null }}</contenidoTabla>
                        <contenidoTabla>{{ p.pivot.fecha_vencimiento ?? null }}</contenidoTabla>
        ...
        <cardLayout>
            <template #tituloCard>
                Tratamientos en los que se implementa
            </template>

            <tablaCRUDLayout>
                <template #tituloTabla>
                    <tituloTabla>Tratamiento</tituloTabla>
                    <tituloTabla>Cantidad a usar</tituloTabla>
                </template>

                <template #contenidoTabla>
                    <tr v-for="(t, index) in tratamientos " :key="index">
                        <checkbox />
                        <contenidoTabla>{{ t.nombre_tratamiento ?? null}}</contenidoTabla>
                        <contenidoTabla>{{ t.pivot.cantidad ?? null}}</contenidoTabla>
                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="flex items-center space-x-4">
                                <button type="button" data-modal-target="updateProductModal"
                                    data-modal-toggle="updateProductModal"
        ...
        <cardLayout>
            <template #tituloCard>
                Citas en los que se usó
            </template>

            <tablaCRUDLayout>
                <template #tituloTabla>
                    <tituloTabla>Cita ID</tituloTabla>
                    <tituloTabla>Cantidad usada</tituloTabla>
                </template>

                <template #contenidoTabla>
                    <tr v-for="(c, index) in citas " :key="index">
                        <checkbox />
                        <contenidoTabla>{{ c.cita_id ?? null}}</contenidoTabla>
                        <contenidoTabla>{{ c.pivot.cantidad ?? null}}</contenidoTabla>
                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="flex items-center space-x-4">
                                <button type="button" data-modal-target="updateProductModal"
                                    data-modal-toggle="updateProductModal"
